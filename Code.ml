...
struct 	
		...
		let compare c1 c2 = if (c1:t) = (c2:t) then 0 else if (c1:t) > (c2:t) then 1 else -1;;
		...
		let code_of_string_bis stringCode = let l = (Str.split (Str.regexp " ") stringCode ) in
					let rec aux acc l = match l with
							| [] -> acc
							| h::t -> aux ((Couleur(h)::acc): t) t in aux [] l;;

		let code_of_string s couleur_probable = if (((code_of_string_bis s)<>[] ) && (contient couleur_probable (code_of_string_bis s))) then Some(code_of_string_bis s)
					else None;;

		let code_2 l1 = let res = List.fold_left (fun acc x -> List.fold_left (fun acc y -> (x::y::[]) :: acc) acc l1) [] l1 in List.rev res;;

		let code_3 l1 = let res = List.fold_left (fun acc x -> List.fold_left (fun acc y -> List.fold_left (fun acc z -> (x::y::z::[]) :: acc) acc l1) acc l1) [] l1 in List.rev res;;

		let code_4 l1 = let res = List.fold_left (fun acc x -> List.fold_left (fun acc y -> List.fold_left (fun acc z ->List.fold_left (fun acc t -> (x::y::z::t::[]) :: acc) acc l1) acc l1) acc l1) [] l1  in List.rev res;;
		...
		let combinaison l = let res = List.fold_left (fun acc x -> List.fold_left (fun acc y -> (x,y) :: acc) acc l) [] l in List.rev res;;
		...
		let rec saisie_code couleur_probable taille_code = print_string "Liste de couleur possible: "; print_list couleur_probable; print_string "\n";
		...
		let decision_final tentativeMax taille_code couleur_probable code_secret = try Sys.command "clear";	
					let rappel = ref "\n" in 
		...
		let reponse_automatique_joueur taille_code couleur_prob tentativeMax = try print_string "Liste de couleur possible: "; print_list couleur_prob; print_string "\n";
											let rappel = ref "\n" in
		...
		let rec saisie_rep_manuel n = try (saisie_rep_manuel2 0 ) with
			...
			let reponse_manuel_joueur taille_code couleur_prob tentativeMax = try print_string "Liste de couleur possible: "; print_list couleur_prob; print_string "\n";
							...
							with 
								|Exit -> let ()=print_string "Vous avez tricher !\n" in true
   								  |Exit_bis -> true;;
			...
			let qui_commence nom_joueur couleur_prob taille_code tentativeMax joueur n autom= match autom with
				...
				let alternance nom_joueur nbPartie couleur_prob taille_code tentativeMax joueur autom=  
						let rec alternance_bis nom_joueur nbPartie couleur_prob taille_code tentativeMax joueur autom score =let ((x,y)) = score in match nbPartie with
											...
											|a when ((a mod 2)=0) -> let b=(qui_commence nom_joueur couleur_prob taille_code tentativeMax joueur 0 autom) in if ((not b ) && (joueur =0)) then 
												 ...
												 else if ((not b) && (joueur =1)) then alternance_bis nom_joueur (nbPartie-1) couleur_prob taille_code tentativeMax joueur autom ((x,y+1))
												 else if ((b) && (joueur =0)) then alternance_bis nom_joueur (nbPartie-1) couleur_prob taille_code tentativeMax joueur autom (x,y+1)
											...
											|a  -> let c = (qui_commence nom_joueur couleur_prob taille_code tentativeMax joueur 1 autom) in (if ((not c) && (joueur =1)) then 
		 															alternance_bis nom_joueur (nbPartie-1) couleur_prob taille_code tentativeMax joueur autom (x+1,y)
				 											 else if ((not c) && (joueur =0)) then alternance_bis nom_joueur (nbPartie-1) couleur_prob taille_code tentativeMax joueur autom (x,y+1)
															 else if ((c) && (joueur =1)) then alternance_bis nom_joueur (nbPartie-1) couleur_prob taille_code tentativeMax joueur autom (x,y+1)
				...
				let mastermind nom_joueur tentativeMax nbPartie autom = let ()=clscreen (Sys.command "clear") in 
					...
					let ()=print_string "Combien de couleurs souhaitez-vous ? (max 6) " in 
					let nbCouleur = read_int () in 
					let couleur_prob = createListCoul nbCouleur in 
					let () = print_string "Quelle taille de code souhaitez-vous ? (max 4) " in 
					let taille_code=read_int () in 
					let joueur=Random.int 2 in 
						victoire nom_joueur npart couleur_prob taille_code tentativeMax joueur autom;;
